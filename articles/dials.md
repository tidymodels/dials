...
[`cost()`](https://dials.tidymodels.org/reference/cost.md), parameter
values are sampled between 1 and 10 and then transformed back to the
original scale by the inverse `-log2()`. So on the original scale, the
...
The [`finalize()`](https://dials.tidymodels.org/reference/finalize.md)
